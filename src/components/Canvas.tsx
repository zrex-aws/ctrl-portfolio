...
export const Canvas = () => {
  ...
  return (
    <main className="relative flex-1 overflow-hidden px-4 pb-20 pt-4">
      <div className="absolute inset-0 bg-grid bg-[size:30px_30px] opacity-40" />
      <div className="win-glass relative z-10 h-full overflow-auto rounded-b-2xl p-5 shadow-panel">
        <div className="mb-6 flex items-end justify-between">
          <h3 className="text-2xl font-semibold">{boardTitles[activeWorkspace]}</h3>
          <p className="max-w-md text-right text-xs text-muted">This portfolio should feel like a product designers use, not a website they browse.</p>
        </div>

        {(activeWorkspace === 'work' || activeWorkspace === 'caseStudies') && (
          <section className="grid gap-4 xl:grid-cols-3">
            {projects.map((project) => (
              <motion.article layout key={project.id} whileHover={{ y: -4 }} className="rounded-2xl border border-white/15 bg-panel/70 p-4">
                ...
                <div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-3 text-xs text-muted">{project.hero}</div>
          ...
          <section className="grid gap-4 md:grid-cols-2">
            {services.map((service) => (
              <button
                ...
                className="rounded-2xl border border-white/15 bg-panel/70 p-4 text-left transition hover:border-accent/50"
              >
        ...
        {activeWorkspace === 'process' && (
          <section className="space-y-3">
            {processFlow.map((step, index) => (
              <div key={step} className="flex items-center gap-3 rounded-xl border border-white/15 bg-panel/65 p-3">
        ...
        {activeWorkspace === 'assets' && (
          <section className="grid gap-4 md:grid-cols-3">
            {['Typography Kit', 'Color Tokens', 'Motion Presets', 'Layout Grids', 'Icon System', 'Voice + Tone'].map((asset) => (
              <div key={asset} className="rounded-2xl border border-white/15 bg-panel/70 p-4">
        ...
        {activeWorkspace === 'clients' && (
          <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {clients.map((client) => (
              <div key={client} className="rounded-2xl border border-white/15 bg-panel/70 p-4 text-center text-sm text-muted hover:text-white">
                {client}
              </div>
            ))}
          </section>
